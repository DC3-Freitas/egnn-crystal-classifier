...
EXP_TO_CLASSIFIERS = {
    "al_fcc": ["DC4", "DC3", "Common Neighbor Analysis (Non-Diamond)", "Interval Common Neighbor Analysis", "Ackland-Jones Analysis"],
    "li_bcc": ["DC4", "DC3", "Common Neighbor Analysis (Non-Diamond)", "Interval Common Neighbor Analysis", "Ackland-Jones Analysis", "VoroTop Analysis"],
    "ti_hcp": ["DC4", "DC3", "Common Neighbor Analysis (Non-Diamond)", "Interval Common Neighbor Analysis", "Ackland-Jones Analysis"],
...
def run_benchmark(exp_name: str) -> None:
    ...
    for test_file in os.listdir(f"egnn_crystal_classifier/benchmarking/md/{exp_name}"):
        ...
        print(f"Processing {data_path} for simulation temperature fraction {sim_temp_id}...")
        try:
            idx = np.where(SIM_TEMPERATURE_FRACTIONS == float(sim_temp_id))[0][0]
        except IndexError:
            print(f"Simulation temperature fraction {sim_temp_id} not found in predefined fractions. Skipping...")
            continue
        ...
        for classifier in classifiers:
            ...
            if classifier == "DC4":
                ...
                acc = compute_heuristic_accuracy(
                    exp_name,
                    data_path,
                    classifier
                )
            ...
            print(f"{classifier} accuracy on {sim_temp_id}: {acc:.2f}, elapsed time: {elapsed_time:.2f}s")
    ...
    df = pd.DataFrame(inference_results, index=SIM_TEMPERATURE_FRACTIONS, columns=classifiers)
    df.to_csv(output_path, index_label="Simulation Temperature Fraction")

    # Save timings
    time_output_path = f"egnn_crystal_classifier/benchmarking/results/{exp_name}_timings.csv"
    time_df = pd.DataFrame(time_results, index=SIM_TEMPERATURE_FRACTIONS, columns=classifiers)
    time_df.to_csv(time_output_path, index_label="Simulation Temperature Fraction")

def main() -> None:
    import argparse
    parser = argparse.ArgumentParser(description="Run benchmarking for crystal structure classification.")
    parser.add_argument("--exp", type=str, choices=["al_fcc", "li_bcc", "ti_hcp", "ge_cd"],
